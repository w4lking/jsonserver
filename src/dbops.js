...
function createRouter(db) {
  ...
  router.get("/api/user/type", (req, res) => {
    ...
    db.query(qryString, [email], (error, results) => {
      ...
      res
        .status(200)
        .json(results[0] || { status: "error", message: "User not found" });
    });
  });
  ...
  router.get("/api/users/unauthenticated", (req, res) => {
    ...
    db.query(qryString, (error, results) => {
      ...
      if (results.length > 0) {
        ...
        res
          .status(404)
          .json({ status: "error", message: "No unauthenticated users found" });
      }
    });
  });
  ...
  router.post("/api/user/authenticate", (req, res) => {
    const { id } = req.body;
    const qryString =
      "UPDATE usuarios SET autenticado = 1 WHERE idusuarios = ?";

    // Primeiro, consulte o e-mail do usuário para enviar a notificação
    const getUserEmailQuery =
      "SELECT email, nome FROM usuarios WHERE idusuarios = ?";

    db.query(getUserEmailQuery, [id], (error, userResults) => {
      if (error) {
        return res
          .status(500)
          .json({ ok: false, status: "error", message: "Database error" });
      }

      if (userResults.length === 0) {
        return res
          .status(404)
          .json({ ok: false, status: "error", message: "User not found" });
      }
  ...
  router.get("/api/user", (req, res) => {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ status: "error", message: "ID is required" }); // Verifica se o ID foi passado
    }
  ...
  router.get("/api/farm", (req, res) => {
    ...
    db.query(qryString, [id], (error, results) => {
      ...
      if (results.length > 0) {
        ...
        res
          .status(404)
          .json({ status: "error", message: "Fazendas not found" });
      }
    });
  });
  ...
  router.post("/api/farm/add", (req, res) => {
    ...
    const qryString =
      "INSERT INTO fazendas (nome, cep, endereco, valor, usuarios_idusuarios) VALUES (?, ?, ?, ?, ?)";
    ...
    db.query(qryString, [nome, cep, endereco, valor, id], (error, results) => {
      if (error) {
        ...
        return res
          .status(500)
          .json({ status: "error", message: "Erro ao adicionar a fazenda." });
      }
  ...
  router.delete("/api/delete", (req, res) => {
    console.log(
      "Recebendo requisição para deletar fazenda com ID:",
      req.query.idfazendas
    ); // Log adicionado
    ...
    db.query(qryString, [id], (error, results) => {
      ...
      if (results.affectedRows > 0) {
        res
          .status(200)
          .json({ status: "success", message: "Fazenda deletada com sucesso" });
      } else {
        res
          .status(404)
          .json({ status: "error", message: "Fazenda não encontrada" });
      }
    });
  });
  ...
  router.get("/api/employees", (req, res) => {
    ...
    const qryString =
      "SELECT * FROM funcionarios WHERE fazendas_usuarios_idusuarios = ?";
    db.query(qryString, [id], (error, results) => {
      if (error) {
        console.error("Database error:", error);
        return res
      ...
      if (results.length > 0) {
        ...
        res.status(404).json({
          status: "error",
          message: "Nenhum funcionario encontrado not found",
        });
      }
    });
  });
  ...
  router.post("/api/employees/add", (req, res) => {
    ...
    const qryString =
      "INSERT INTO funcionarios (nome, cpf, email, telefone, salario, senha, fazendas_idfazendas, fazendas_usuarios_idusuarios) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    db.query(
      qryString,
      [nome, cpf, email, telefone, salario, senha, idfazendas, idusuario],
      (error, results) => {
        if (error) {
          console.error("Erro ao adicionar funcionario:", error);
          return res.status(500).json({
            status: "error",
            message: "Erro ao adicionar a funcionario.",
          });
        }
  ...
  router.delete("/api/employees/delete", (req, res) => {
    console.log(
      "Recebendo requisição para deletar empregado com ID:",
      req.query.id
    ); // Log adicionado
    const id = req.query.id;
    if (!id) {
    ...
    db.query(qryString, [id], (error, results) => {
      ...
      if (results.affectedRows > 0) {
        ...
        res
          .status(404)
          .json({ status: "error", message: "Funcionario não encontrado" });
      }
    });
  });
  ...
  router.post("/api/user/login", (req, res) => {
    ...
    const qryString =
      "SELECT idusuarios, email, senha, perfil, autenticado, bloqueado FROM usuarios WHERE email = ?";
    db.query(qryString, [email], async (error, results) => {
      ...
      if (isCorrectPass) {
        ...
        const token = jwt.sign(
          { id: user.idusuarios, perfil: user.perfil },
          "seu-segredo-jwt",
          { expiresIn: "1h" }
        );
    ...
    db.query(
      ...
      (error, results) => {
        if (error) {
          ...
          return res
            .status(500)
            .json({ status: "error", message: "Erro ao verificar o usuário" });
        }
        ...
        if (results.length > 0) {
          ...
          return res
            .status(409)
            .json({ status: "error", message: message.trim() });
        }
